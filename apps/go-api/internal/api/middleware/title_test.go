...
func TestTitleExtractor_EnglishLocale(t *testing.T) {
	for _, header := range []string{"", "en", "en-US", "fr", "fr-FR", "de"} {
		...
		req := httptest.NewRequest(http.MethodGet, "/test", nil)
		if header != "" {
			req.Header.Set("X-LevelUp-Locale", header)
		}
		handler.ServeHTTP(httptest.NewRecorder(), req)
		if captured != "en" {
			t.Errorf("X-LevelUp-Locale=%q → locale %q, want %q", header, captured, "en")
		}
	}
}
