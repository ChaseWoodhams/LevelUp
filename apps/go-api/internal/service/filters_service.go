...
func localizeExperienceOptions(opts []domain.LabelValue, _ string) {
	for i := range opts {
		opts[i].Label = opts[i].Value
	}
}

func experienceTypeOptionsForLocale(values []string, _ string) []domain.LabelValue {
	if len(values) == 0 {
		return nil
	}
	opts := make([]domain.LabelValue, len(values))
	for i, v := range values {
		opts[i] = domain.LabelValue{Label: v, Value: v}
	}
	return opts
}
...
func (s *FiltersService) Resolve(
	...
	localizeExperienceOptions(resolved.AvailableOptions.ExperienceTypes, "en")
...
func ResolveFiltersFromRowsAt(
	...
	// Cascade values use the canonical English labels directly.
...
func migrateCascadeFromRows(_ []domain.FilterMatchRow, effective domain.FilterContextInput) domain.FilterContextInput {
	return effective
}
...
// Matche par SUBSTRING sur la VALUE (FR canonique « Unranked PvP » / « PVP
...
func emptyResolved(effective domain.FilterContextInput, sess domain.SessionOptions) domain.FilterContextResolved {
	// Label and value share the English-only experience category.
